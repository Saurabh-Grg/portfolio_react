...
// ─── Helper: convert relative /uploads/... paths to full backend URL ──────────
...
const AdminProfile = () => {
  ...
  const handleUploadAvatar = () => {
    ...
    uploadAvatar(selectedFile, {
      ...
      onError: (err: any) => {
        toast({ title: 'Error', description: err?.response?.data?.error || err?.message || 'Failed to upload avatar', variant: 'destructive' });
      },
    });
  };
  ...
  const handleDeleteAvatar = () => {
    ...
    deleteAvatar(undefined, {
      onSuccess: () => {
        toast({ title: 'Success', description: 'Profile picture deleted successfully!' });
        setSelectedFile(null);
        setPreviewUrl('');
        refetch();
      },
      onError: (err: any) => {
        toast({ title: 'Error', description: err?.response?.data?.error || err?.message || 'Failed to delete avatar', variant: 'destructive' });
      },
    });
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      updateProfile(formData, {
        ...
        onError: (err: any) => {
          toast({ title: 'Error', description: err?.response?.data?.message || err?.message || 'Failed to save profile', variant: 'destructive' });
        },
      });
    } catch (err) {
      console.error('❌ [AdminProfile] Unexpected error:', err);
  ...
  // Resolved avatar URL — handles both relative /uploads/... and absolute https://...
  const resolvedAvatarUrl = getAvatarUrl(formData.avatar_url);
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="name">Full Name</Label>
                <Input id="name" name="name" value={formData.name || ''} onChange={handleInputChange} disabled={isUpdating} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="title">Professional Title</Label>
                <Input id="title" name="title" value={formData.title || ''} onChange={handleInputChange} disabled={isUpdating} />
              </div>
            </div>

            <div className="space-y-2">
              <Label htmlFor="bio">Biography</Label>
              <Textarea id="bio" name="bio" value={formData.bio || ''} onChange={handleInputChange} rows={5} disabled={isUpdating} />
              <p className="text-xs text-muted-foreground">
                Write a brief description about yourself, your experience, and what you specialize in.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="email">Email Address</Label>
                <Input id="email" name="email" type="email" value={formData.email || ''} onChange={handleInputChange} disabled={isUpdating} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="location">Location</Label>
                <Input id="location" name="location" value={formData.location || ''} onChange={handleInputChange} disabled={isUpdating} />
              </div>
            </div>

            <div className="space-y-2">
              <Label htmlFor="phone">Phone Number</Label>
              <Input id="phone" name="phone" value={formData.phone || ''} onChange={handleInputChange} disabled={isUpdating} />
            </div>
          </CardContent>
        </Card>
              ...
              <div className="flex flex-col items-center justify-center space-y-4">
                ...
                <div className="w-32 h-32 rounded-full overflow-hidden bg-muted border-2 border-dashed border-gray-300 flex items-center justify-center">
                  {previewUrl ? (
                    ...
                    // Saved avatar — uses getAvatarUrl to prefix backend base URL
                    <img
                      src={resolvedAvatarUrl}
                      alt={formData.name || 'Profile'}
                      className="w-full h-full object-cover"
                      onError={(e) => {
                        console.warn('⚠️ Avatar failed to load:', resolvedAvatarUrl);
                ...
                {!previewUrl && (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground mt-2">Supported: JPEG, PNG, GIF, WebP</p>
                  </div>
                )}
                ...
                {previewUrl && (
                  <div className="w-full space-y-2">
                    ...
                    <div className="flex gap-2 w-full">
                      <Button onClick={handleUploadAvatar} disabled={isUploadingAvatar || isDeletingAvatar} className="flex-1">
                        {isUploadingAvatar ? (
                      ...
                      <Button onClick={handleClearSelection} disabled={isUploadingAvatar || isDeletingAvatar} variant="outline">
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
                ...
                {/* Delete button */}
                {resolvedAvatarUrl && !previewUrl && (
                  <Button onClick={handleDeleteAvatar} disabled={isUploadingAvatar || isDeletingAvatar} variant="destructive" className="w-full">
                    {isDeletingAvatar ? (
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input id="github" name="github_url" value={formData.github_url || ''} onChange={handleInputChange} placeholder="https://github.com/yourusername" disabled={isUpdating} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="linkedin" className="flex items-center gap-2">
                <Linkedin className="h-4 w-4" /> LinkedIn
              </Label>
              <Input id="linkedin" name="linkedin_url" value={formData.linkedin_url || ''} onChange={handleInputChange} placeholder="https://linkedin.com/in/yourusername" disabled={isUpdating} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="twitter" className="flex items-center gap-2">
                <Twitter className="h-4 w-4" /> Twitter
              </Label>
              <Input id="twitter" name="twitter_url" value={formData.twitter_url || ''} onChange={handleInputChange} placeholder="https://twitter.com/yourusername" disabled={isUpdating} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="website">Website</Label>
              <Input id="website" name="website_url" value={formData.website_url || ''} onChange={handleInputChange} placeholder="https://yourwebsite.com" disabled={isUpdating} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex gap-3 justify-end">
        ...
        <Button onClick={handleSaveProfile} disabled={isUpdating || !hasChanges} className="min-w-[120px]">
          {isUpdating ? (
